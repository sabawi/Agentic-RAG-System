...
def generate_pdta_plot_image(stock,start_date,fast,slow,lookback):
    ...
    last_date,recommendation, df_trend_pdta, ema_days_at_buy = calculate_pdta_alphatrend(
            stock=stock,
            df=df1,
            fast_ema=fast,  # Changed from Fast_EMA
            slow_ema=slow,  # Changed from Slow_EMA
            lookback=lookback   # Changed from Lookback, assuming this exists
    )
